...
from nets.magnet import intra_proposal,inter_proposal
...
class Network(nn.Module):
  def __init__(self, batch_size=1):
    ...
    self.magnet_intra = intra_proposal(2048,cfg.POOLING_SIZE,cfg.POOLING_SIZE)
    self.magnet_inter = inter_proposal(2048,cfg.POOLING_SIZE,cfg.POOLING_SIZE)
  ...
  def train_step(self, blobs, train_op):
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'], blobs['gt_masks'])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss = self._losses["rpn_cross_entropy"].item(), \
                                                                        self._losses['rpn_loss_box'].item(), \
                                                                        self._losses['cross_entropy'].item(), \
                                                                        self._losses['loss_box'].item(), \
                                                                        self._losses['loss_mask'].item(), \
