...
def eval_attributes(loader, model, split, opt):
    ...
    while True:
        ...
        _, _, att_scores, _, _ = model.sub_encoder(Feats['pool5'], Feats['fc7'], fake_phrase_emb)
...
def eval_split(loader, model, crit, split, opt):
    ...
    while True:
        ...
        for i, sent_id in enumerate(sent_ids):
            ...
            # weights : (n, 2) weights on modulessub_encoder
            # att_scores: (n, num_atts)
            tic = time.time()
            scores, sub_grid_attn, sub_attn, loc_attn, rel_attn, rel_ixs, weights, att_scores, _, _ = \
            ...
            # if used up
            if num_sents > 0 and loss_evals >= num_sents:   # aaaaaaaaaa loss_eval add one s
                finish_flag = True
                break
