...
def main(args):
    ...
    file_name = '%s_%s_%s_ann_feats1.h5' % (args.net_name, args.imdb_name, args.tag)
    ...
    #     ann_ids = image['ann_ids']
    #     for ann_id in ann_ids:
    #         # ann_feat_path = osp.join('/home/ssd/refer/feats/matt_res101/umd_refercocog/ann_feat',
    #         #                          '{}.h5'.format(ann_id))
    #         ann_feat_path = osp.join('/media/ext2/wangshuai/refer/feats/matt_res101/umd_refercocog/ann_feat',
    #                                  '{}.h5'.format(ann_id))
    #         f = h5py.File(ann_feat_path, 'w')
    #
    #         ann = loader.Anns[ann_id]
    #         ann_pool5, ann_fc7 = ann_to_pool5_fc7(mrcn, ann, net_conv, im_info)
    #         # ann_h5_id = ann['h5_id']
    #         f.create_dataset('pool5', data=ann_pool5[0].data.cpu().numpy())
    #         f.create_dataset('fc7', data=ann_fc7[0].data.cpu().numpy())
    #         f.close()
    #
    #         # pool5_set[ann_h5_id] = ann_pool5.data.cpu().numpy()
    #         # fc7_set[ann_h5_id] = ann_fc7.data.cpu().numpy()
    #         # print(ann_pool5.data.cpu().numpy().shape)
    #         # print(ann_fc7.data.cpu().numpy().shape)
    #         # exit()
    #     # if i % 20 == 0:
    #     #   print('%s/%s done.' % (i+1, len(images)))
    #
