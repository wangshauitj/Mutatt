...
torch.cuda.manual_seed_all(24)  # if you are using multi-GPU.
np.random.seed(24)  # Numpy module.
random.seed(24)  # Python random module.
torch.manual_seed(24)
...
# checkpoint = torch.load('output/refcocog_umd/lock_loc_phrase_guided.pth')
# checkpoint2 = torch.load('output/refcocog_umd/test109_first.pth')
# checkpoint3 = torch.load('output/refcocog_umd/test109_first_2.pth')
# checkpoint4 = torch.load('output/refcocog_umd/lock_bbox_guided_add.pth')
# torch.nn.Module.dump_patches = True
# checkpoint = torch.load('/home/lizhongguo/src/MAttNet/output/refcocog_umd/beifen_1.pth')
# checkpoint2 = torch.load('/home/lizhongguo/src/MAttNet/output/refcocog_umd/test99-2.pth')
# parameter = checkpoint['model']
# pa2 = checkpoint2['model']
# a = 1
# exit()
...
def lossFun(loader, optimizer, model, mm_crit, att_crit, opt, iter):
    ...
    # forward
    tic = time.time()
    scores, _, sub_attn, loc_attn, rel_attn, _, _, att_scores, kl_loss, text_logits = model(Feats['pool5'], Feats['fc7'],
                                                                                 Feats['lfeats'], Feats['dif_lfeats'],
                                                                                 Feats['cxt_fc7'], Feats['cxt_lfeats'],
                                                                                 labels, att_labels, select_ixs)
    loss = mm_crit(scores)  # tensor(0.2030, device='cuda:0')
    # loss = loss + kl_loss
    ...
    if select_ixs.numel() > 0:
        ...
        # loss += opt['att_weight'] * att_crit(image_logits.index_select(0, select_ixs),
        #                                     att_labels.index_select(0, select_ixs))
        # loss += opt['att_weight'] * att_crit(text_logits.index_select(0, select_ixs),
        #                                      att_labels.index_select(0, select_ixs))
        # loss += cmpc_loss
    # if iter < 500:
...
def main(args):
    ...
    while True:
        ...
        if iter % opt['save_checkpoint_every'] == 0 or iter == opt['max_iters']:
            val_loss, acc, predictions, overall = eval_utils.eval_split(loader, model, None, 'val', opt)    # modify by ws
            ...
            if best_val_score is None or current_score > best_val_score:
                ...
                # none 00 phrase 01 bbox 10 phrase+bbox 11 m--modify
                checkpoint_path = osp.join(checkpoint_dir, 'tmp.pth')
                checkpoint = {}
            ...
            # with open(osp.join(checkpoint_dir, opt['id'] + '.json'), 'w') as io: out_method_seed_0.json
            with open(osp.join(checkpoint_dir, 'tmp.json'), 'wb') as io:
                json.dump(infos, io, cls=MyEncoder)  # before shixiong # this part
                # io.write(iter); io.write('\n')
                # io.write(epoch); io.write('\n')
                # io.write(val_accuracies); io.write('\n')
                # io.write(best_val_score); io.write('\n'); io.write('\n')
