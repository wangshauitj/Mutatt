...
2. anns:       [{ann_id, category_id, image_id, box, h5_id}]    # all boxes in dataset
3. sentences:  [{sent_id, tokens, h5_id}]
4. word_to_ix: {word: ix}
5. att_to_ix : {att_wd: ix}
6. att_to_cnt: {att_wd: cnt}count of every att (sum)
7. label_length: L
...
class GtMRCNLoader(Loader):
    ...
    def prepare_mrcn(self, head_feats_dir, args):
        ...
        # original code does not have this part, i dont know its goal why shixiong write it
        # feats_h5 = '/home/lizhongguo/src/MAttNet/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime/img_feats2.h5'
        # f = h5py.File(feats_h5, 'w')
        # pool5_set = f.create_group('pool5')
        # fc7_set = f.create_group('fc7')
        # for image_id, image in tqdm(self.Images.items()):
        #   pool5, fc7 = self.getImageFeat(image_id)
        #   # print(image_id, pool5.shape, fc7.shape)
        #   # exit()
        #   pool5_set.create_dataset(str(image_id), data=pool5.cpu().detach().numpy())
        #   fc7_set.create_dataset(str(image_id), data=fc7.cpu().detach().numpy())
    ...
    def getBatch(self, split, opt):
        ...
        for i in range(batch_size):
            ...
            batch_image_ids += [image_id]  # 15 ge image_id / batch

        # fetch feats
        batch_ref_ids = []  # ref / batch , num 24 > image id num 15
        ...
        for image_id in batch_image_ids:
            ref_ids = self.Images[image_id][
                'ref_ids']  # some bboxs of all bboxs in one image (have corresponding  sents)
            batch_ref_ids += self.expand_list(ref_ids, seq_per_ref)  # ref * 3, [1,2]->[1,1,1,2,2,2]

            # fetch head and im_info
            head, im_info = self.image_to_head(image_id)  # feat in image size
            ...
            for ref_id in ref_ids:
                ref_ann_id = self.Refs[ref_id]['ann_id']  # ref_id -- ann_id  one to one  y-->x

                # pos ids
                pos_ann_ids = [ref_ann_id] * seq_per_ref  # [x] -> [x,x,x]
                pos_sent_ids = self.fetch_sent_ids_by_ref_id(ref_id, seq_per_ref)  # [a,b,c] all correaponding x
        ...
        pos_fc7 = torch.cat(batch_pos_fc7, 0);  # [3,...] [6....] [3...] sum 15 -> [72,...]
        pos_fc7.detach()
    ...
    def sample_neg_ids(self, ann_id, seq_per_ref, sample_ratio):  # random process need be read carefully
        """Return
    ...
    def image_to_head(self, image_id):
        ...
        # feats_h5 = osp.join('/home/lizhongguo/src/MAttNet/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime', str(image_id)+'.h5')
        ...
        for i, ref_ann_id in enumerate(ref_ann_ids):
            ...
            if len(ref['att_wds']) > 0:
                ...
                for wd in ref['att_wds']:
                    ...
                    # labels[i] = self.att_to_ix[wd]  # question 1-1 or 1-n classification
    ...
    def getImageFeat(self, image_id):
        # fetch head and im_info
        image = self.Images[image_id]
        head, im_info = self.image_to_head(image_id)
        head = Variable(torch.from_numpy(head).cuda())
